...
class GroqProvider(
    ...
    override val id: String = AIProviderType.GROQ.id
    override val type: AIProviderType = AIProviderType.GROQ
    override var config: ProviderConfig = ProviderConfig(type = type, priorityOrder = 1)
    ...
    override suspend fun generateResponse(
        ...
        try {
            ...
            val url = config.customEndpointUrl ?: "https://api.groq.com/openai/v1/chat/completions"

            val messagesArray = JSONArray()
            messagesArray.put(
                JSONObject().put("role", "system")
                    .put("content", "You are VoidCore, an intelligent, concise, on-device assistant. Speak directly and naturally.")
            )
            ...
            GenerationResponse(
                ...
                latencyMs = latency,
                isSuccess = false,
                errorMessage = "No response choice returned from Groq"
            )
        } catch (e: Exception) {
            val latency = System.currentTimeMillis() - startTime
            GenerationResponse(
